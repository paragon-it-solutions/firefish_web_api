...
export default function AddSkillModal({ candidateId, candidateName }) {
  ...
  const handleSkillChange = (event, newValue) => {
    ...
    const key = newValue.key.startsWith('.') ? newValue.key.substring(1) : newValue.key;
  ...
  const handleAddSkill = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Failed to add skill. Please try again.', severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
